...
$fc_lists_images_attached = $args['fc_lists_images_attached'] ?? false;
...
<section class="section images-carousel media-carousel<?= $css_class ?>">
    <?php if($fc_lists_images_attached) : ?><div class="images-left"><?php endif; ?>
            ...
            <div class="w-swiper swiper">
                <div class="swiper-wrapper<?= $swiper_class ?>" data-options='<?= $swiper_data ?>'>
                    ...
                    while ( $slides_query->have_posts() ) : $slides_query->the_post();
                        ...
                        $main_image = $ACF_banner->main_image ?? '';
                        ...
                        <div class="item">
                            <div class="overlay">
                                ...
                                <picture class="<?=$_video_class?>">
                                    ...
                                    <source srcset="<?= Helper::attachmentImageSrc( $responsive_image, 'medium' ) ?>" media="(max-width: 576px)">
                                    <source srcset="<?= Helper::attachmentImageSrc( $responsive_image, 'medium' ) ?>" media="(max-width: 768px)">
                                    <?php elseif(has_post_thumbnail()) : ?>
                                    <source srcset="<?= Helper::postImageSrc( $post->ID, 'medium' ) ?>" media="(max-width: 576px)">
                                    <source srcset="<?= Helper::postImageSrc( $post->ID, 'medium' ) ?>" media="(max-width: 768px)">
                                    <?php elseif($main_image) : ?>
                                    <source srcset="<?php echo wp_get_attachment_image_url($main_image, 'medium'); ?>" media="(max-width: 576px)">
                                    <source srcset="<?php echo wp_get_attachment_image_url($main_image, 'medium'); ?>" media="(max-width: 768px)">
                                    <?php endif; ?>
                                    ...
                                    <?php } elseif($main_image) { ?>
                                        <source srcset="<?php echo wp_get_attachment_image_url($main_image, 'large'); ?>" media="(max-width: 1024px)">
                                        <source srcset="<?php echo wp_get_attachment_image_url($main_image, 'medium'); ?>" media="(max-width: 1280px)">
                            ...
                            $button_name = $ACF_banner->button_name ?? '';
                            ...
                            <div class="overlay-content">
                                <div class="inner">
                                    ...
                                    echo Helper::ACF_Link_Wrap($button_name, $button_link, 'button-link' );
                                    ?>
                                </div>
                            </div>
                            <?php endif; ?>
    ...
    <?php if($fc_lists_images_attached) : ?></div><?php endif; ?>
    <?php if($fc_lists_images_attached) : ?>
        <div class="images-right">
            <ul>
                <?php foreach($fc_lists_images_attached as $lists) :?>
                    <li>
                        <figure class="images">
                            <?php echo Helper::iconImage( $lists['images'], 'large' ); ?>
                        </figure>
                        <div class="contents">
                            <span><?php echo $lists['title'] ?></span>
                            <span><?php echo $lists['title_sub'] ?></span>
                        </div>
                        <?php echo Helper::ACF_Link( $lists['links'], 'attached-link' ); ?>
                    </li>
                <?php endforeach; ?>
            </ul>
        </div>
