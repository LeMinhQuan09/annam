...
if ( ! function_exists( '__hd_credits' ) ) {
	...
	function __hd_credits(): void {
		...
		$copyright = sprintf(
			'<div class="text"><span class="copyright">&copy; %1$s %2$s</span><span class="hd">, %3$s <a class="_blank" title="%6$s" href="%4$s" %5$s>%6$s</a></span></div>',
			date( 'Y' ), // phpcs:ignore
			get_bloginfo( 'name' ),
		...
		<img src="<?php echo get_template_directory_uri(); ?>'/assets/img/payments.png'" class="img-payment" alt="">
		<?php echo '</div>';
	}
}
