...
final class Shortcode {
	...
	private function init(): void {
		$shortcodes = [
			...
			'productcat_menu' => __CLASS__ . '::productcat_menu',
			'vertical_menu'   => __CLASS__ . '::vertical_menu',
			'footer_menu' => __CLASS__ . '::footer_menu',
	 ...
	 * @return string
	 */
	public static function footer_menu( array $atts = [] ): string {
		$atts = shortcode_atts(
			[
				'location' => 'footer-nav',
				'class'    => 'footer-menu',
		...
		$location = $atts['location'] ?: 'footer-nav';
		$class    = $atts['class'] ? Helper::esc_attr_strip_tags( $atts['class'] ) : 'footer-menu';
		$depth    = $atts['depth'] ? absint( $atts['depth'] ) : 1;
		$id       = $atts['id'] ?: esc_attr( uniqid( 'menu-', false ) );

		return Helper::horizontalNav( [
			'menu_id'        => $id,
			'menu_class'     => 'menu horizontal footer-menu ' . $class,
			'theme_location' => $location,
			'depth'          => $depth,
			'echo'           => false,
		] );
	 ...
	 * @return string
	 */
	public static function productcat_menu( array $atts = [] ): string {
		$atts = shortcode_atts(
			[
				'location' => 'productcat-nav',
				'class'    => 'productcat-menu',
				'id'       => esc_attr( uniqid( 'menu-', false ) ),
				'depth'    => 4,
			],
			$atts,
		...
		$location = $atts['location'] ?: 'productcat-nav';
		$class    = $atts['class'] ? Helper::esc_attr_strip_tags( $atts['class'] ) : 'productcat-menu';
		$depth    = $atts['depth'] ? absint( $atts['depth'] ) : 1;
		$id       = $atts['id'] ?: esc_attr( uniqid( 'menu-', false ) );

		return Helper::horizontalNav( [
			'menu_id'        => $id,
			'menu_class'     => 'menu vertical productcat-menu ' . $class,
			'theme_location' => $location,
			'depth'          => $depth,
			'echo'           => false,
		] );
